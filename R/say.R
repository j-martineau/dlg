...
say <- function(..., lev = 0, sub = 0, clear = FALSE) {
  if (clear) {dlg::xcon()}
  lev <- dlg::f0(ppp::.cmp_nnw_scl(lev), lev, 0)
  sub <- base::min(2, dlg::f0(ppp::.cmp_nnw_scl(sub), sub, 0))
  if      (lev == 0) {pref <- ""}
  else if (lev == 1) {pref <- "\n"}
  else               {pref <- dlg::g0(base::c("\n", base::rep("| ", lev - 1)))}
  if (sub == 1) {
    pref <- dlg::p0(pref, " > ")
    suff <- ""
  } else if (sub == 2) {
  ...
  msg <- dlg::p0(dlg::av(...), collapse = "")
  if (msg == "") {msg <- "."}
  msg <- dlg::p0(pref, msg, suff)
  base::cat(msg)
}
