...
say <- function(..., .lev = 0, .sub = 0, .clear = FALSE) {
  ...
  .lev <- uj::f0(ppp::cmp_nnw_scl(.lev), .lev, 0)
  .sub <- base::min(2, uj::f0(ppp::cmp_nnw_scl(.sub), .sub, 0))
  if      (.lev == 0) {Prefix <- ""}
  else if (.lev == 1) {Prefix <- "\n"}
  else                {Prefix <- uj::g0(base::c("\n", uj::r("| ", .lev - 1)))}
  if (.sub == 1) {
    Prefix <- uj::p0(Prefix, " > ")
    Suffix <- ""
  } else if (.sub == 2) {
  ...
  Message <- uj::p0(uj::av(...), collapse = "")
  if (Message == "") {Message <- "."}
  Message <- uj::p0(Prefix, Message, Suffix)
  base::cat(Message)
}
...
#' @export
cat0 <- function(..., .clear = FALSE) {dlg::say("\n", ..., "\n", .clear = .clear)}

#' @describeIn console_basics Prints `"Hit [enter] or [return] to continue"` to the console and pausse execution until the user hits the enter or return key.
#' @export
