...
alert <- function(..., title = "alert", sub = "", ps = "", def = "", ft = "r|w|b", fs = "k|y|p", fm = "", fp = "k|y|i", d = " ", clear = FALSE) {
  fmt <- function(x, f) {
    if (f == "") {return(x)}
    f <- dlg::av(base::strsplit(f, "|", fixed = TRUE))
    dlg::txt(dlg::p0(" ", x, " "), bg = f[1], fg = f[2], st = f[3])
  }
  msg   <- dlg::p0(base::as.character(dlg::av(...)), collapse = d)
  title <- dlg::p0(base::toupper(base::as.character(dlg::av(title))), collapse = d)
  sub   <- dlg::p0(base::as.character(dlg::av(sub)), collapse = d)
  ps    <- dlg::p0(base::as.character(dlg::av(ps)), collapse = d)
  def   <- dlg::p0(base::as.character(dlg::av(def)), collapse = d)
  ft    <- dlg::f0(!ppp::.cmp_chr(ft), "r|w|b", dlg::p0(ft, collapse = "|"))
  fs    <- dlg::f0(!ppp::.cmp_chr(fs), "k|y|i", dlg::p0(fs, collapse = "|"))
  fp    <- dlg::f0(!ppp::.cmp_chr(fp), "k|y|i", dlg::p0(fp, collapse = "|"))
  fm    <- dlg::f0(!ppp::.cmp_chr(fm), "k|w|p", dlg::p0(fm, collapse = "|"))
  if (msg == "") {msg <- def}
  errs <- NULL
  if (!dlg::ok_fmt(ft)     ) {errs <- base::c(errs, "[ft] must be consistent with the description the [specifying formats] section of the [alert] topic of package [dlg] (?alert).")}
  if (!dlg::ok_fmt(fs)     ) {errs <- base::c(errs, "[fs] must be consistent with the description the [specifying formats] section of the [alert] topic of package [dlg] (?alert).")}
...
chooseN <- function(opts, ..., n = NULL, min = NULL, max = NULL, all = base::is.null(c(n, max)), none = FALSE, ft = "r|w|b", fs = "k|y|p", d = " ", clear = FALSE) {
  errs <- NULL
  if (!ppp::unq_atm_mvc(opts)                                ) {errs <- base::c(errs, "[opts] must be a unique atomic multivec (?unq_atm_mvc).")}
  if (!ppp::.cmp_lgl_scl(all)                                ) {errs <- base::c(errs, "[all] must be TRUE or FALSE.")}
  if (!ppp::.cmp_lgl_scl(none)                               ) {errs <- base::c(errs, "[none] must be TRUE or FALSE.")}
  ...
  okLo     <- dlg::f0(!defN  , T, n   >= 1    )
  okHi     <- dlg::f0(!defN  , T, n   <= nOpts)
  okMin    <- dlg::f0(!defMin, T, min <= nOpts)
  okMax    <- dlg::f0(!defMax, T, max <= nOpts)
  okCombo  <- (!defMin & !defMax) | !defN
  okMinMax <- dlg::f0(!defMin | !defMax, T, min <= max)
...
NO <- function(..., ft = "r|w|b", fs = "k|y|p", d = " ", clear = FALSE) {
  msg <- dlg::p0(dlg::av(...), collapse = d)
  dlg::choose_from(base::c("yes", "no"), msg, F, F, 1, 1, ft, fs, "NO", dlg::callers(), clear) == "no"
}
...
YES <- function(..., ft = "r|w|b", fs = "k|y|p", d = " ", clear = FALSE) {
  msg <- dlg::p0(dlg::av(...), collapse = d)
  dlg::choose_from(base::c("yes", "no"), msg, F, F, 1, 1, ft, fs, "YES", dlg::callers(), clear) == "yes"
}
...
OK <- function(..., ft = "r|w|b", fs = "k|y|p", d = " ", clear = FALSE) {
  msg <- dlg::p0(dlg::av(...), collapse = d)
  dlg::choose_from(base::c("ok", "cancel"), msg, F, F, 1, 1, ft, fs, "OK", dlg::callers(), clear, can = F) == "ok"
}
...
CANCEL <- function(..., ft = "r|w|b", fs = "k|y|p", d = " ", clear = FALSE) {
  msg <- dlg::p0(dlg::av(...), collapse = d)
  dlg::choose_from(base::c("ok", "cancel"), msg, F, F, 1, 1, ft, fs, "CANCEL", dlg::callers(), clear, can = F) == "cancel"
}
...
ask_new <- function(old, type = "replacement values", u = TRUE, sub = "", ft = "r|w|b", fs = "k|y|p", fm = "", fp = "k|y|i", d = "|", clear = FALSE) {
  ...
  D <- dlg::f0(d == "|", "pipe (|)",
       ...
       dlg::f0(d == "^", "caret (^)",
       dlg::f0(d == " ", "space",
       dlg::f0(d == "`", "backtick (`)", dlg::p0("'", d, "'")))))))))
  lst <- dlg::p0(old, collapse = dlg::p0(" ", d, " "))
  qst <- dlg::p0("Enter a %s separated list of %d %s for the following %s separated original values:\n\n  %s\n\n(enter '{cancel}' to cancel)\n")
  qst <- base::sprintf(qst, D, base::length(old), type, D, lst)
  dlg::alert(qst, title = "response required", ps = "ENTER YOUR RESPONSE", sub = "", ft = ft, fs = fs, fm = fm, fp = fp, d = "", clear = clear)
  ans  <- base::readline()
  ans  <- dlg::av(base::strsplit(ans, d, fixed = T))
